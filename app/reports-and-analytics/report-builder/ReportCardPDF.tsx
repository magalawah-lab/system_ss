...
export default function ReportCardPDF() {
  ...
  const title = search.get("title") || "END OF TERM ASSESSMENT REPORT";
  const term = search.get("term") || "2";
  ...
  // Calculate report rows for each subject
  const reportRows = useMemo(() => {
    return (stream?.subjects ?? [])
      .filter((subEntry: any) => {
        ...
        if (isOptional) {
          return student?.optionalSubjects.includes(subjectName);
        }
      ...
      // Get scores for this subject and student
      const c1Score =
        c1Assessment?.subjectScores?.[subjectName]?.[studentId] ??
        c1Assessment?.scores?.[studentId];
      const c2Score =
        c2Assessment?.subjectScores?.[subjectName]?.[studentId] ??
        c2Assessment?.scores?.[studentId];
      const endOfCycleScore =
        endOfCycleAssessment?.subjectScores?.[subjectName]?.[studentId] ??
        endOfCycleAssessment?.scores?.[studentId];
  ...
  }, [stream, student, c1Assessment, c2Assessment, endOfCycleAssessment, catalog]);

  // Calculate overall grade and comments
  const overallGrades = reportRows
    .map((r) => r.hundred)
    .filter((h) => h !== "");
  const overallPercentage =
    overallGrades.length > 0
      ? (overallGrades.reduce((a: number, b: string | number) => a + (Number(b) || 0), 0)) /
  ...
  return (
    ...
    <div className="print-container">
      ...
      <div className="report-title">
        <h2>{title}</h2>
        <p className="report-subtitle">TERM {term}, {year}</p>
      ...
      <div className="comments-section">
        ...
        <div className="report-footer">
          <p><strong>Next Term:</strong>September 14, 2026 — December 4, 2026</p>
        </div>
      </div>
