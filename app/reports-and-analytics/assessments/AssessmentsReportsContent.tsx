...
import { useSchoolData, AssessmentReport, SubjectAuditReport, Assessment, Student } from "../../context/SchoolDataContext";
...
export default function AssessmentsReports() {
  ...
  const [selectedAuditAssessmentId, setSelectedAuditAssessmentId] = useState<string>('');
  ...
  // Get the global class index for the current class
  ...
  const classAssessments = useMemo((): AssessmentWithScores[] => {
    ...
    return (currentClass.assessments || []).filter((a: any) => {
      ...
      // If assessment doesn't have year/term but we're in current year/term, show it
      // This handles legacy assessments that were migrated
      return false;
    });
  }, [currentClass, currentAcademicYearId, currentTermId, showAllAssessments]);

  // Get unassigned assessments (those without year/term)
  const unassignedAssessments = useMemo((): AssessmentWithScores[] => {
    if (!currentClass) return [];
    return (currentClass.assessments || []).filter((a: any) => 
  ...
  const [selectedPrintAssessmentId, setSelectedPrintAssessmentId] = useState<string>('');

  function gradeFromPercent(p: number | null): string {
    if (p === null) return '—';
    if (p >= 90) return 'A';
    if (p >= 80) return 'B';
    if (p >= 70) return 'C';
  ...
  const studentAverages: StudentScore[] = (currentStream?.students ?? []).map((stu: Student) => {
    ...
    const valid = scores.filter((p: number | null): p is number => typeof p === 'number');
    return { 
      id: stu.id, 
      avg: valid.length ? valid.reduce((s: number, v: number) => s + v, 0) / valid.length : null 
    };
  });

  const sorted = studentAverages
    .slice()
    .filter((s: StudentScore): s is StudentScore & { avg: number } => s.avg !== null)
    .sort((a: StudentScore & { avg: number }, b: StudentScore & { avg: number }) => (b.avg - a.avg));
  
  const ranks: Record<string, number> = {};
  sorted.forEach((s: StudentScore & { avg: number }, i: number) => { 
    ranks[s.id] = i + 1; 
  });

  const classAverage = ((): number | null => {
    const vals = studentAverages
      .map((s: StudentScore) => s.avg)
  ...
  function exportStudentReportCSV(student: Student): void {
    if (!currentClass) return;
    const rows: string[] = [];
    ...
    for (const a of streamAssessments) {
      ...
      // Convert all values to strings and escape if needed
      const escapedRow = rowData.map((value: string | number): string => {
  ...
  // Helper to get year and term names for an assessment
  const getAssessmentYearTerm = (assessment: any): YearTermInfo => {
    let yearName = '—';
    let termName = '—';
    ...
    return { yearName, termName, isUnassigned };
  };
  ...
  return (
    <div style={{ padding: '1rem', maxWidth: '1400px', margin: '0 auto' }}>
          ...
          <table className="simple-table" style={{ width: '100%', marginTop: 8, borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {reports.map((r: AssessmentReport) => {
                ...
                const { yearName, termName, isUnassigned } = assessment ? getAssessmentYearTerm(assessment) : { yearName: '—', termName: '—', isUnassigned: true };
      ...
      <div style={{ marginTop: 18 }}>
        <h3>Missing Scores Audit</h3>
        <p className="muted">Subjects with completely missing scores for the selected assessment.</p>
        {reports.length === 0 ? (
          <p className="muted">No assessments to audit.</p>
        ) : (
          <>
            <div style={{ marginBottom: 12 }}>
              <label htmlFor="audit-assessment">Select assessment to audit: </label>
              <select
                id="audit-assessment"
                value={selectedAuditAssessmentId}
                onChange={(e) => setSelectedAuditAssessmentId(e.target.value)}
                style={{ marginLeft: 8, padding: '0.4rem', borderRadius: '4px', border: '1px solid #ddd' }}
              >
            ...
            {selectedAuditAssessmentId && (() => {
              const globalIndex = classes.findIndex((c: any) => c.name === currentClass?.name && c.level === currentClass?.level);
              const auditReports: SubjectAuditReport[] = globalIndex >= 0 ? getSubjectsWithMissingScoresForAudit(globalIndex, selectedAuditAssessmentId) : [];
              
              return (
                <div>
                  {auditReports.length === 0 ? (
                    <p className="muted" style={{ fontStyle: 'italic', color: '#22c55e' }}>✓ All subjects have at least some scores entered for this assessment.</p>
                  ) : (
                    <table className="simple-table" style={{ width: '100%', marginTop: 8, borderColor: '#ef4444', borderCollapse: 'collapse' }}>
                      <thead style={{ backgroundColor: '#fef2f2' }}>
                        <tr>
                          <th style={{ padding: '0.5rem', textAlign: 'left', borderBottom: '1px solid #e5e7eb' }}>Class</th>
                          <th style={{ padding: '0.5rem', textAlign: 'left', borderBottom: '1px solid #e5e7eb' }}>Stream</th>
                          <th style={{ padding: '0.5rem', textAlign: 'left', borderBottom: '1px solid #e5e7eb' }}>Subject</th>
                          <th style={{ padding: '0.5rem', textAlign: 'left', borderBottom: '1px solid #e5e7eb' }}>Teacher Responsible</th>
                          <th style={{ padding: '0.5rem', textAlign: 'left', borderBottom: '1px solid #e5e7eb' }}>Students</th>
                        </tr>
                      </thead>
                      <tbody>
                        {auditReports.map((audit: SubjectAuditReport, idx: number) => (
                          <tr key={idx} style={{ backgroundColor: '#fff5f5', borderLeft: '4px solid #ef4444' }}>
                            <td style={{ padding: '0.5rem' }}>
                              <strong>{audit.className}</strong>
                              <div style={{ fontSize: '0.85em', color: '#666' }}>({audit.classLevel}-Level)</div>
                            </td>
                            <td style={{ padding: '0.5rem' }}>{audit.streamName}</td>
                            <td style={{ padding: '0.5rem' }}>{audit.subjectName}</td>
                            <td style={{ padding: '0.5rem' }}>
                              <div>{audit.teacherName}</div>
                              {audit.teacherId && (
                                <div style={{ fontSize: '0.85em', color: '#666' }}>ID: {audit.teacherId}</div>
                              )}
                            </td>
                            <td style={{ padding: '0.5rem', textAlign: 'center' }}>
                              <strong style={{ color: '#ef4444' }}>{audit.totalStudents}</strong>
                            </td>
                          </tr>
                        ))}
                      </tbody>
          ...
          <div>
            {streamAssessments.length === 0 ? (
              ...
              <div style={{ overflow: 'auto' }}>
                <table style={{ width: '100%', marginTop: 8, borderCollapse: 'collapse', minWidth: '800px' }}>
                  <thead>
                    <tr style={{ background: '#f9fafb' }}>
                      ...
                      {streamAssessments.map((a: any) => (
                        <th key={a.id} style={{ padding: '0.5rem', textAlign: 'center', borderBottom: '1px solid #e5e7eb' }}>{a.name}</th>
                      ))}
                      <th style={{ padding: '0.5rem', textAlign: 'center', borderBottom: '1px solid #e5e7eb' }}>Average (%)</th>
                  ...
                  <tbody>
                    {currentStream.students.map((stu: Student, i: number) => {
                      const scores = streamAssessments.map((a: any) => ({ 
                        score: a.scores?.[stu.id] ?? null, 
                        max: a.maxScore 
                      }));
                      const percentValues = scores.map((s: { score: number | null; max: number | undefined }) => 
                        s.score === null ? null : (s.max ? (s.score / s.max) * 100 : s.score)
                      );
                      const validPercents = percentValues.filter((p: number | null): p is number => typeof p === 'number');
                      const avgPercent = validPercents.length 
                        ? (validPercents.reduce((s: number, v: number) => s + v, 0) / validPercents.length) 
                        : null;
                      ...
                      return (
                        <tr key={stu.id} style={{ borderBottom: '1px solid #e5e7eb' }}>
                          ...
                          {scores.map((s: { score: number | null; max: number | undefined }, idx: number) => (
            ...
            {modal?.type === 'view' && (
              <div className="modal-backdrop" role="dialog" aria-modal="true" onClick={() => setModal(null)}>
                <div className="modal printable" onClick={(e) => e.stopPropagation()}>
                  ...
                  <table style={{ width: '100%', marginTop: 8, borderCollapse: 'collapse' }}>
                    ...
                    <tbody>
                      {streamAssessments.map((a: any) => {
                        ...
                        const { yearName, termName } = getAssessmentYearTerm(a);
                        return (
                  ...
                  <div style={{ marginTop: 12, display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                    ...
                    {streamAssessments.length > 0 && (
                      <select 
                        value={selectedPrintAssessmentId} 
                        onChange={(e) => setSelectedPrintAssessmentId(e.target.value)} 
                        style={{ padding: '0.4rem', borderRadius: '4px', border: '1px solid #ddd' }}
                      >
                    ...
                    <button 
                      style={{ padding: '0.4rem 1rem', border: '1px solid #ddd', borderRadius: '4px', background: 'white', cursor: 'pointer' }} 
                      onClick={() => {
                        const globalIndex = classes.findIndex((c: any) => c.name === currentClass?.name && c.level === currentClass?.level);
                        const url = `/reports-and-analytics/assessments/print?classIndex=${globalIndex}&streamIndex=${selectedStreamIndex}&studentId=${modal.student.id}${selectedPrintAssessmentId ? `&assessmentId=${selectedPrintAssessmentId}` : ''}`;
